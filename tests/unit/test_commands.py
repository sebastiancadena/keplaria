...
def test_command_id_is_deterministic():
    assert command_id("CASE-1", "create_supplier") == "CASE-1:create_supplier"
    assert command_id("CASE-1", "create_supplier") == command_id("CASE-1", "create_supplier")
...
def test_claim_after_success_is_refused_and_returns_the_external_id(db, case_id):
    claim_command(db, case_id, "create_supplier", PAYLOAD)
    record_success(db, case_id, "create_supplier", "Andes Verde SAS", {"ok": True})
...
def test_claim_after_failure_is_reacquired_and_counts_attempts(db, case_id):
    claim_command(db, case_id, "create_supplier", PAYLOAD)
    record_failure(db, case_id, "create_supplier", "HTTP 503")

    claim = claim_command(db, case_id, "create_supplier", PAYLOAD)

    assert claim.acquired is True
    assert get_command(db, case_id, "create_supplier")["attempts"] == 2
...
def test_record_success_persists_the_external_id(db, case_id):
    ...
    stored = get_command(db, case_id, "create_supplier")
