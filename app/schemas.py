...
class CanonicalEvent(BaseModel):
    ...
    # A present-but-blank department ("") is refused at every schema
    # version, not just v2+: a v1 producer that has already adopted the
    # `department` field but emits it config-defaulted to "" would otherwise
    # resolve through `case.get("department") or None` (app/nodes.py) to
    # `None`, which grandfathers onto the catalog's widest legacy scope —
    # the one path in this branch that is permissive rather than
    # fail-closed. `None` (the field genuinely absent) still grandfathers;
    # only a present empty/whitespace string is rejected.
    ...
    def _require_department_at_v2(self) -> "CanonicalEvent":
        if self.department is not None and not self.department.strip():
            raise ValueError(
                "department, if present, must be non-blank at any schema_version"
            )
