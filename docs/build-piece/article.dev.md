...
description: Three measured findings from an unattended agent build — a safety filter that goes blind at 108 characters, a thinking budget that is an off switch, and a smoke test that lied.
---
...
- **Finding one.** The commercial safety filter I enabled could not see the one attack I most needed it to catch — and its blindness turned out to have a measurable location, down to a character count.
...
## Finding one: the filter that stopped detecting at 108 characters
...
What was left had to be the surrounding text itself. The filter sees the payload block when it stands alone and misses it inside the document — so somewhere between "alone" and "full certificate" it goes blind, and finding *where* is a bisection: glue increasing amounts of the certificate's real prose in front of the payload, re-scan, repeat.
...
Eighty-nine characters is about a line and a half of text. That is all the legitimate context this classifier tolerates before a borderline attack disappears into it. The mechanism has a name — context dilution — and an analogy: a guard who spots the pickpocket in an empty lobby but not in a crowd. The crowd, here, is your own document. And "borderline" matters: the crude jailbreak survived three times that much prose without flickering, while my realistic payload never scored above LOW confidence even standing alone. Remember that my template was already at its most paranoid setting. **There is no configuration of this filter that catches this payload inside a real document.**

Score one for the small scanner — but keep the score honest, in both directions. My scanner is a heuristic tuned against a representative fixture; rephrase the payload and it walks past. That is exactly why it was never the load-bearing defence: in my system, a document flagged as tainted is never shown to any agent at all, and its case is forced to a blocked verdict — so a tainted document cannot cause an ERP write no matter what any model thinks of it. Detection is best-effort; enforcement is structural. And Model Armor is not one filter: its malicious-URL check caught a known-bad link appended to a certificate with zero false positives on clean documents — that one delivers. Its sensitive-data filter in basic mode found a credit-card number and missed an email address, a phone number, and a US Social Security number on the same page. A mixed instrument.
...
You are probably thinking what I was thinking: *he hit his rate limit.* So I went and looked. Vertex publishes per-model quotas, and on my project the numbers for the two models are identical — same defaults, no special caps on either. A limit that 3.6 never touched cannot explain 3.7 failing on the same workload. Whatever this was, it was not my allowance.
...
Each probe took an afternoon or less, lives in the repository next to the evidence it produced, and re-runs from a single command — because the second-cheapest thing after writing a probe is re-running it, and a measurement you cannot re-run has a shelf life you will not notice expiring.

---

*Disclosure: this piece describes work built as an entry in a public AI agent hackathon run by Google Cloud and Devpost, and is written by the entrant. It is published as part of that entry and is eligible for the competition's bonus content points. All supplier names, certificates, and watchlist entries referenced in the system are synthetic fixtures authored for the project. No customer or production data is involved. Portions of the code and this article were written with AI assistance, disclosed in the project's provenance ledger.*
